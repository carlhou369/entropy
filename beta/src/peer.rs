...
/// Default max outbound streams
const DEFAULT_MAX_OUTBOUND_STREAM: usize = 30;

/// Default max inbound streams
const DEFAULT_MAX_INBOUND_STREAM: usize = 10;

/// Save fragment save in FRAGMENT_REPLIC_CNT peers
const FRAGMENT_REPLIC_CNT: usize = 1;
...
struct PeerState {
    ...
    outbound_stream_sema: Arc<Semaphore>, // outbound stream restriction
    inbound_stream_sema: Arc<Semaphore>,  // inbound stream restriction
}
    ...
    pub fn new(peer_opt: PeerOpt, p2p_client: Client) -> Self {
        ...
        PeerState {
            ...
            inbound_stream_sema: Arc::new(Semaphore::new(
                peer_opt
                    .max_inbound_stream
                    .unwrap_or(DEFAULT_MAX_INBOUND_STREAM),
            )),
        }
    }
}
...
async fn put(
    ...
    for (chunk_id, chunk) in chunks.into_iter() {
        ...
        for (fragment_id, fragment) in chunk.into_iter() {
            ...
            // todo: consider full node save local copy
            ...
            for peer in closest.clone().into_iter() {
                ...
                set.spawn(async move {
                    ...
                    let _fragment_cid = cid(&fragment_clone);
...
async fn get(
    ...
    info!("get content cid {}", cid.0.clone());
    ...
    while let Some(res) = set.join_next().await {
        ...
        let buf = match read_local_content(fragment_cid.clone()) {
            Ok(buf) => buf,
            Err(e) => {
                debug!("read local fragment content error {}", e);
                continue;
            },
        };
...
impl Default for PeerOpt {
    fn default() -> Self {
        Self {
            ...
            max_inbound_stream: Some(DEFAULT_MAX_INBOUND_STREAM),
            max_oubound_stream: Some(DEFAULT_MAX_OUTBOUND_STREAM),
        }
    }
}
